...
export async function sign(
  ...
  const addr =
    device.startsWith("ledger") && coinType !== 60
...
export function formatTokenDenom(tokenDenom) {
  if (tokenDenom && tokenDenom.code === undefined) {
    ...
    if (denom.toLowerCase().startsWith("factory/")) {
      const factoryDenom = denom.split("/").pop().toUpperCase();
      return factoryDenom.startsWith("U")
        ? factoryDenom.slice(1)
        : factoryDenom;
    }
