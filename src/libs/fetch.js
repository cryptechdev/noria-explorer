...
export default class ChainFetch {
  ...
  async getLatestBlock(config = null) {
    ...
    if (ver && compareVersions(ver, "0.45") < 1) {
      return this.get("/blocks/latest", config)
  ...
  async getStakingDelegatorDelegation(delegatorAddr, validatorAddr) {
    return this.get(
      ...
      StakingDelegation.create(commonProcess(data).delegation_response)
    );
  }
  ...
  async getStakingPool() {
    return this.get("/cosmos/staking/v1beta1/pool").then((data) =>
      new StakingPool().init(commonProcess(data.pool))
    );
  }
  ...
  async getMintingInflation() {
    ...
    if (this.config.chain_name === "echelon") {
      return this.get("/echelon/inflation/v1/inflation_rate").then((data) =>
        Number(data.inflation_rate / 100 || 0)
      );
    }
    if (this.isModuleLoaded("minting")) {
      return this.get("/cosmos/mint/v1beta1/inflation").then((data) =>
        Number(commonProcess(data.inflation))
      );
    }
    return 0;
  }
  ...
  async getValidatorUnbondedList() {
    return this.get(
      "/cosmos/staking/v1beta1/validators?pagination.limit=100&status=BOND_STATUS_UNBONDED"
    ).then((data) => {
  ...
  async getStakingValidator(address) {
    return this.get(`/cosmos/staking/v1beta1/validators/${address}`).then(
      (data) => new Validator().init(commonProcess(data).validator)
    );
  }

  async getSlashingParameters() {
    if (this.isModuleLoaded("slashing")) {
      return this.get("/cosmos/slashing/v1beta1/params").then((data) =>
        commonProcess(data.params)
      );
    }
    return null;
  ...
  async getMintParameters() {
    ...
    if (this.config.chain_name === "echelon") {
      const result = await this.get("/echelon/inflation/v1/params").then(
        (data) => data.params
      );
    ...
    if (this.isModuleLoaded("minting")) {
      return this.get("/cosmos/mint/v1beta1/params").then((data) =>
        commonProcess(data.params)
      );
    }
    return null;
  ...
  async getGovernanceParameterDeposit() {
    return this.get("/cosmos/gov/v1beta1/params/deposit").then((data) =>
      commonProcess(data.deposit_params)
    );
  }

  async getGovernanceParameterTallying() {
    return this.get("/cosmos/gov/v1beta1/params/tallying").then((data) =>
      commonProcess(data.tally_params)
    );
  }

  async getGovernanceParameterVoting() {
    return this.get("/cosmos/gov/v1beta1/params/voting").then((data) =>
      commonProcess(data.voting_params)
    );
  }

  async getGovernanceTally(pid, total, conf) {
    return this.get(`/cosmos/gov/v1beta1/proposals/${pid}/tally`, conf).then(
      (data) => new ProposalTally().init(commonProcess(data).tally, total)
    );
  }
  ...
  async getGovernanceDeposits(pid) {
    ...
    return this.get(`/cosmos/gov/v1beta1/proposals/${pid}/deposits`).then(
      (data) => {
        const result = commonProcess(data);
        return Array.isArray(result)
          ? result.reverse().map((d) => new Deposit().init(d))
  ...
  async getGovernanceVotes(pid, next = "", limit = 50) {
    if (compareVersions(this.config.sdk_version, "0.40") < 0) {
      return this.get(`/cosmos/gov/v1beta1/proposals/${pid}/votes`).then(
        (data) => ({
  ...
  async getAuthAccount(address, config = null) {
    return this.get(
      "/cosmos/auth/v1beta1/accounts/".concat(address),
      config
    ).then((data) => {
      const result = commonProcess(data);
      return result;
  ...
  async getBankAccountBalance(address) {
    return this.get("/cosmos/bank/v1beta1/balances/".concat(address)).then(
      (data) => commonProcess(data).balances
    );
  }
  ...
  async getValidatorSlashs(address, config = null) {
    return this.get(
      `/cosmos/distribution/v1beta1/validators//${address}/slashes`,
      config
    ).then((data) => commonProcess(data));
  }
  ...
  async getStakingValidators(address) {
    return this.get(
      `/cosmos/distribution/v1beta1/delegators/${address}/validators?pagination.size=200`
    ).then((data) => commonProcess(data.validators));
  }
  ...
  async getBankBalances(address, config = null) {
    return this.get(
      "/cosmos/bank/v1beta1/balances/".concat(address),
      config
    ).then((data) => commonProcess(data));
  }

  async getCommunityPool(config = null) {
    return this.get("/cosmos/distribution/v1beta1/community_pool", config).then(
      (data) => commonProcess(data)
    );
  }
  ...
  async getMarketChart(days = 14, coin = null) {
    ...
    if (conf.assets[0] && conf.assets[0].coingecko_id) {
      return ChainFetch.fetch(
        ...
        `/api/v3/coins/${
          coin || conf.assets[0].coingecko_id
  ...
  async getCoinInfo(coin = null) {
    ...
    if (conf.assets[0] && conf.assets[0].coingecko_id) {
      return ChainFetch.fetch(
        " https://api.coingecko.com",
        `/api/v3/coins/${coin || conf.assets[0].coingecko_id}`
      );
    }
    return null;
  }
  ...
  async broadcastTx(bodyBytes, config = null) {
    const txbytes = bodyBytes.authInfoBytes
      ? TxRaw.encode(bodyBytes).finish()
      : bodyBytes;
  ...
  static fetch(host, url) {
    const ret = fetch((Array.isArray(host) ? host[0] : host) + url).then(
