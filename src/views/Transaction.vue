<template>
  <div>
    ...
    <b-card v-if="error === null" title="Basic" class="text-truncate">
    ...
    <b-card v-if="tx.logs" :title="`Events (total: ${tx.logs.length})`" no-body>
      <b-card-body
  ...
  created() {
    ...
    this.$http
      ...
      .then((res) => {
        ...
        console.log("tx", res);
        this.tx = res;
        this.tx.logs = JSON.parse(res.raw_log);
      })
      .catch((err) => {
        this.error = err;
      });
